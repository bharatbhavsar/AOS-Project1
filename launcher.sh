...
do
	if [ $node_id -ne $rootNode ]; then	
		host=${host_names[$node_id]}
		neighbors=${neighbors_dict["$node_id"]}
		ssh -o StrictHostKeyChecking=no $netid@$host "java $PROG $node_id \"$nodes_location\" \"$neighbors\" \"$config_file_name\"" &
	fi
done
ssh -o StrictHostKeyChecking=no $netid@${host_names[$rootNode]} "java $PROG $rootNode \"$nodes_location\" \"${neighbors_dict["$rootNode"]}\" \"$config_file_name\" \"1\"" & 
